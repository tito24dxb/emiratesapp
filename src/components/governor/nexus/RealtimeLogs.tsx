  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xl font-bold text-gray-900">Real-time Audit Logs</h2>
        {!loading && (
          <span className="ml-auto text-xs text-gray-500">{events.length} total events</span>
        )}
      </div>
      ...
      <div className="space-y-2 max-h-96 overflow-y-auto">
        {loading ? (
          ...
          <div className="text-center py-8 bg-gray-50/50 rounded-lg backdrop-blur-sm">
          ...
          sortedEvents.map((event, index) => (
            <motion.div
              ...
              className="p-3 bg-white/50 rounded-lg border border-gray-200/50 hover:bg-white/80 transition"
            >
              <div className="flex items-start gap-3">
                <div className={`px-2 py-1 rounded-md text-xs font-semibold ${getEventColor(event.eventType)}`}>
                  {event.eventType}
                </div>
